...
void TM_MFRC522_WriteRegister(uint8_t addr, uint8_t val) {
	...
	//}
	//Send data
	TM_MFRC522_CS_Write(GPIO_PIN_RESET); // CS low
	transmitStatus = HAL_SPI_Transmit(&hspi1, buffer, 2, HAL_MAX_DELAY);
	if (transmitStatus != HAL_SPI_ERROR_NONE) {
		TM_MFRC522_CS_Write(GPIO_PIN_SET); //CS high
		handleError();
	}
...
uint8_t TM_MFRC522_ReadRegister(uint8_t addr) {
	...
	//transmitStatus = HAL_SPI_Transmit(&hspi1, &addr, 2, HAL_MAX_DELAY);
	//if (transmitStatus != HAL_SPI_ERROR_NONE) {
	//	handleError();
	//}

	//uint8_t dummy = MFRC522_DUMMY;
	//transmitStatus = HAL_SPI_Receive(&hspi1, &retval, 2, HAL_MAX_DELAY);
...
void TM_MFRC522_SetBitMask(uint8_t reg, uint8_t mask) {
	uint8_t temp;
	temp = TM_MFRC522_ReadRegister(reg);
	TM_MFRC522_WriteRegister(reg, temp | mask);
}
...
TM_MFRC522_Status_t TM_MFRC522_CalculateCRC(uint8_t *pIndata, uint8_t len,
		uint8_t *pOutData) {
	uint8_t i, n;
...
void TM_MFRC522_CS_Write(GPIO_PinState val) {
	HAL_GPIO_WritePin(RFID_SS_GPIO_Port, RFID_SS_Pin, val);
}
